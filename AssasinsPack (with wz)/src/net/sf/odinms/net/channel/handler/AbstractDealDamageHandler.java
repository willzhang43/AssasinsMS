...
public abstract class AbstractDealDamageHandler extends AbstractMaplePacketHandler {
	...
	private int PVP_MAP = 109020001;
	public static class AttackInfo {
		public int numAttacked, numDamage, numAttackedAndDamage;
		public int skill, stance, direction, charge, success, mobid;
			...
			if (monster != null) {
				...
				if (distance > 360000.0) { // 600^2, 550 is approximatly the range of ultis
					player.getCheatTracker().registerOffense(CheatingOffense.ATTACK_FARAWAY_MONSTER, Double.toString(Math.sqrt(distance)));
					 ...
					 //AutobanManager.getInstance().addPoints(player.getClient(), 50, 120000, "Exceeding attackrange");
				}
	...
	public AttackInfo parseDamage(LittleEndianAccessor lea, boolean ranged) {
		...
		ret.skill = lea.readInt();
		                if (ret.skill == 1121001 || ret.skill == 1221001 || ret.skill == 1321001) {
	                ret.mobid = lea.readInt();
	                ret.success = lea.readByte();
