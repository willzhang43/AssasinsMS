...
*/
 ...
 public class EnterMTSHandler extends AbstractMaplePacketHandler {
       ...
       if ((c.getPlayer().getHp() < 1)) {
           new ServernoticeMapleClientMessageCallback(5, c).dropMessage("Not while dead.");
           c.getSession().write(MaplePacketCreator.enableActions());
       }else {
             if ((c.getPlayer().getMapId() < 910000000) || (c.getPlayer().getMapId() > 910000022)){
               new ServernoticeMapleClientMessageCallback(5, c).dropMessage("You will be transported to the Free Market Entrance right away.");
               c.getSession().write(MaplePacketCreator.enableActions());
                 MapleMap to;
                 MaplePortal pto;
