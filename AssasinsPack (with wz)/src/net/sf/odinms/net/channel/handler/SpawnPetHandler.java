...
public class SpawnPetHandler extends AbstractMaplePacketHandler {
	...
	public void unequipPet(MapleClient c) {
		try {
			...
			ps.setString(1, c.getPlayer().getPet().getName()); // Set name
			ps.setInt(2, c.getPlayer().getPet().getLevel()); // Set Level
			ps.setInt(3, c.getPlayer().getPet().getCloseness()); // Set Closeness
			ps.setInt(4, c.getPlayer().getPet().getFullness()); // Set Fullness
			ps.setInt(5, c.getPlayer().getPet().getUniqueId()); // Set ID
