			...
			if (c.getPlayer().isAlive()) {
				if (skill.getId() != 2311002 || c.getPlayer().canDoor()) {
					skill.getEffect(skillLevel).applyTo(c.getPlayer(), pos);
                                       ...
                                       MaplePacket monstermagnetpacket = MaplePacketCreator.showMagnet(mobid, success); 
                                        c.getPlayer().getMap().broadcastMessage(monstermagnetpacket);
                                        MaplePacket monstermagnetpacket2 = MaplePacketCreator.showMonsterMagnet2(c.getPlayer().getId(),skill.getId(),1, direction); 
                                        c.getPlayer().getMap().broadcastMessage(monstermagnetpacket2);
                                        c.getSession().write(MaplePacketCreator.enableActions());
                                    }
			if(skill.getId() != 1121001 && skill.getId() != 1221001 && skill.getId() != 1321001){
				} else {
