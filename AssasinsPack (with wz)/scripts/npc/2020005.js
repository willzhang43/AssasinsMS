...
function action(mode, type, selection) {
	...
	if (status == 0 && mode == 1) {
		...
		var items = new Array(2050003,2050004,4006000,4006001);
		var costs = new Array(300,400,5000,5000);
		for (var i = 0; i < items.length; i++){
			selStr += "\r\n#L" + i + "##z" + items[i] + "# (" + costs[i] + " meso)#l#b";
		}
		...
		if (cm.getMeso() < cost * selection)
		{
			...
			if (cm.canHold(item)) {
				...
				cm.sendOk("I'm afraid you do not have enough room for my items.");
			}
		}
		cm.dispose();
	}
