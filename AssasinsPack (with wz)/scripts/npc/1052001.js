		...
		if (status == 0) {
			if (cm.getJob().equals(net.sf.odinms.client.MapleJob.BEGINNER)) {
				...
				if (cm.getLevel() >= 30 
					...
					cm.sendOk("You have chosen stupidly.");
					cm.dispose();
				}
			}
		} else if (status == 1) {
			cm.sendNextPrev("It is an important and final choice. You will not be able to turn back I AM WILL.");
		} else if (status == 2) {
			cm.sendYesNo("Do you want to become a #rThief#k?");
		} else if (status == 3) {
			if (cm.getJob().equals(net.sf.odinms.client.MapleJob.BEGINNER))
				cm.changeJob(net.sf.odinms.client.MapleJob.THIEF);
			cm.sendOk("So be it! Now go, and go with JOSH.");
			cm.dispose();
		} else if (status == 11) {
			cm.sendNextPrev("You may be ready to take the next step as a #rAssassin#k or #rBandit#k.");
		} else if (status == 12) {
			cm.sendAcceptDecline("But first I must test your skills. Are you ready? NO");
		} else if (status == 13) {
			if (cm.haveItem(4031011)) {
				cm.sendOk("Please report this bug at http://odinms.de/forum/.\r\nstatus = 13");
			} else {
				cm.startQuest(100009);
				cm.sendOk("Take this letter to the #bJob Instructor#k somewhere in the city. He will show you the way DONT GET MARBLES talkto himagain.");
			}
		} else if (status == 21) {
			cm.sendSimple("What do you want to become padawan?#b\r\n#L0#Assassin#l\r\n#L1#Bandit#l#k");
		} else if (status == 22) {
			var jobName;
			if (selection == 0) {
				jobName = "Assassin";
