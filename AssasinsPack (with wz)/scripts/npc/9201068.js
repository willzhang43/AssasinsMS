...
/* NLC ticket gate
	Warp to NLC waiting platform (600010002)
...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (mode == 1)
			...
			status--;
		if (status == 0) {
			cm.sendNext("Hi, I'm the New Leaf City ticket gate.");
		} else if (status == 1) {
			cm.sendNextPrev("I can take you to the subway train to Kerning City, if you have a ticket.\r\n(I think Bell sells them)")
		} else if (status == 2) {
