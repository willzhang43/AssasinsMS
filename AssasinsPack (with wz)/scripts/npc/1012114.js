/*
Edited by Frantier from dev-odinms and ragezone
*/
/*
...
function action(mode, type, selection) {
	if (mode == -1) {
		...
		if (status >= 0 && mode == 0) {
			cm.sendOk("Ok talk to me when your ready");
			cm.dispose();
			return;
		}
		if (mode == 1)
			...
			status--;
		if (status == 0) {
				cm.sendSimple("I'm so hungry!\r\n#L0#I brought you a #bMoon Bunny Rice Cake.#k#l\r\n#L1#Send me out of here!.#l.");
			}
		else if (status == 1) {
			if (selection == 0) {
			if (cm.haveItem(4001101,5)) {
			cm.sendOk("Moon Bunny Rice Cakes! Thanks! Here this is for you.");
			}
			else {
			cm.sendOk("Your gonna need #b5 of those Moon Bunny Rice Cakes.#k");
			cm.dispose();
			}
			} 
		else if (selection == 1) {
