...
function enter(pi) {
	...
	var avail = eim.getProperty("2stageclear");
	if (avail == null) {
	// do nothing; maybe send a message to the player?
	pi.getPlayer().getClient().getSession().write(MaplePacketCreator.serverMessage(int 6, int 0, "The warp is currently unavailable."));
	}
	else {
		pi.getPlayer().changeMap(target, targetPortal);
	}
