	...
	public void givePartyItems(int id, short quantity, List<MapleCharacter> party) {
		for(MapleCharacter chr : party) {
			...
			if (quantity >= 0) {
				...
				MapleInventoryManipulator.removeById(cl, MapleItemInformationProvider.getInstance
().getInventoryType(id), id, -quantity, true, false);
   ...
   public boolean warp(int map, String curPortal, String nextPortal) {
       ...
       {
           c.getSession().write(MaplePacketCreator.serverNotice(5, "The battle against the boss has begun. The portal is now closed. Please try again later."));
           return false;
   }
 }
}
